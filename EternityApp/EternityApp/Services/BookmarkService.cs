...
namespace EternityApp.Services
{
    public class BookmarkService
    {
        ...
        //Получаем список закладок достопримечательностей
        public async Task<IEnumerable<AttractionBookmark>> GetAttractionBookmarkList(int userId)
        {
            string result = await _client.GetStringAsync(_url + "attractionbookmark/" + userId);
            return JsonSerializer.Deserialize<IEnumerable<AttractionBookmark>>(result, _options);
        }
        ...
        // Добавить закладку достопримечательности
        public async Task AddAttractionBookmark(AttractionBookmark bookmark)
        {
            await _client.PostAsync(_url + "attractionbookmark", new StringContent(JsonSerializer.Serialize(bookmark), Encoding.UTF8, "application/json"));
        }
        ...
        // Удаление закладки достопримечательности
        public async Task DeleteAttractionBookmark(int id)
        {
            await _client.DeleteAsync(_url + "attractionbookmark/" + id);
        }
    }
}
